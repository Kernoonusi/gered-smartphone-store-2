...
export function ProductCard({ item }: { item: SmartphoneFull }) {
  ...
  return (
    <Card className="group relative flex h-full flex-col justify-between w-full max-w-xs overflow-hidden rounded-2xl border border-white/30 bg-white/10 shadow-lg backdrop-blur-md transition-all duration-300 hover:border-white/50 hover:shadow-2xl">
      ...
      <div className="absolute -top-16 -right-16 -z-10 h-40 w-40 rounded-full bg-indigo-400/20 blur-xl"></div>
      <div className="absolute -bottom-16 -left-16 -z-10 h-32 w-32 rounded-full bg-blue-300/20 blur-xl"></div>

      {/* Favorites button */}
      <Button
        variant="outline"
        size="icon"
        className="absolute top-2 right-2 rounded-full border-indigo-200/70 bg-white/50 backdrop-blur-sm hover:bg-indigo-50/80 hover:text-indigo-700"
        onClick={onAddToFavorites}
      >
        <Heart className={`h-4 w-4 ${isInFavorites ? 'fill-indigo-500 text-indigo-500' : 'text-indigo-500'}`} />
      </Button>
      ...
      <CardContent className="p-4">
        ...
        <a
          ...
          className="group/image mb-5 flex h-52 items-center justify-center overflow-hidden rounded-lg p-2"
        >
        ...
        <div className="mb-4 space-y-2.5">
          ...
          <p className="text-2xl font-bold text-gray-200">{currencyFormatter.format(item.price)}</p>
        </div>
      ...
      <CardFooter className="flex justify-center px-4 mt-auto">
        {!isInCart ? (
          <button
            onClick={onAddToCart}
            className="grid grid-cols-[auto_auto_1fr_auto] w-full place-content-center gap-2 rounded-xl border border-white/10 bg-indigo-500/80 px-6 py-4 text-white shadow-lg backdrop-blur-md transition-all hover:bg-indigo-600/80"
          ...
          <div className="grid grid-cols-[auto_auto_auto] w-full place-content-center gap-4 rounded-xl border border-white/10 bg-indigo-500/80 px-6 py-4 text-white shadow-lg backdrop-blur-md">
